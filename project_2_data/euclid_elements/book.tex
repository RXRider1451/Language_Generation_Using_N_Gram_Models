...
\usepackage[reqno]{amsmath}		\usepackage{amssymb}			\usepackage[english]{babel}		\usepackage[utf8]{inputenc}		\usepackage{graphicx}                   \usepackage{makeidx}			\usepackage{wrapfig}			\usepackage{needspace}			\newcounter{wrapwidth}
\makeindex
...
[\textsc{i.}]. Bisect the angle\index{Angle!bisection of} $ACB$ by the
...
$DCF$ is equal to the angle $ECF$, and they are adjacent angles. Therefore (Def.~\textsc{xiii}.) each of them is a right
...
nearest to the line through\label{throuyh} the centre is greater than every
...
greater than $FOP$; \emph{therefore} [I.\ \textsc{xxiv.}] \emph{the base $GP$ is
